...
int disk_buffer_segment_wrap_needed(seg, endblkno)
segment *seg;
int endblkno;
{
...
void disk_buffer_segment_wrap(seg, endblkno)
segment *seg;
int endblkno;
{
...
int disk_buffer_overlap(seg, curr)
segment *seg;
ioreq_event *curr;
{
   int tmp;
...
void disk_buffer_remove_from_seg(currdiskreq)
diskreq *currdiskreq;
{
...
/* return real or effective owner of segment or NULL.  For effective, a 
   ...
   is less than the OWNER's outblkno
*/

diskreq* disk_buffer_seg_owner(seg, effective)
segment *seg;
int effective;
{
...
int disk_buffer_reusable_segment_check(currdisk, currseg)
disk *currdisk;
segment *currseg;
{
...
int disk_buffer_recyclable_segment_check(currdisk, currseg, isread)
disk *currdisk;
segment *currseg;
int isread;
{
...
segment* disk_buffer_recyclable_segment(currdisk, isread)
disk *currdisk;
int isread;
{
...
int disk_buffer_block_available(currdisk, seg, blkno)
disk *currdisk;
segment *seg;
int blkno;
{
...
void disk_interferestats(currdisk, curr)
disk *currdisk;
ioreq_event *curr;
{
   if (!disk_printinterferestats) {
      return;
   }
...
void disk_buffer_stats(currdisk, curr, seg, hittype)
disk *currdisk;
ioreq_event *curr;
segment *seg;
int hittype;
...
segment* disk_buffer_select_read_segment(currdisk, currdiskreq)
disk *currdisk;
diskreq *currdiskreq;
{
   segment *seg;
...
segment* disk_buffer_select_write_segment(currdisk, currdiskreq)
disk *currdisk;
diskreq *currdiskreq;
{
   segment *seg;
...
segment * disk_buffer_select_segment(currdisk, currdiskreq, set_extradisc)
disk *currdisk;
diskreq *currdiskreq;
int set_extradisc;
{
   ...
   if (set_extradisc && currdisk->extra_write_disconnect && 
       !currdisk->neverdisconnect && !currdisk->extradisc_diskreq && 
       !(currdiskreq->ioreqlist->flags & READ) && seg && 
...
int disk_buffer_check_read_segments(currdisk, currioreq, buffer_reading)
disk *currdisk;
ioreq_event *currioreq;
int* buffer_reading;
{
...
int disk_buffer_check_write_segments(currdisk, currioreq)
disk *currdisk;
ioreq_event *currioreq;
{
   segment *seg;
...
fflush(outputfile);
   ...
   while (seg) {
	 ...
	 return(BUFFER_NOMATCH);
      ...
      } else if ((seg->state == BUFFER_CLEAN) || (seg->state == BUFFER_DIRTY)) {
         /* check for collision with uncleanable data */
	 if (seg->diskreqlist) {
	       if (disk_buffer_overlap(seg,currioreq) &&
		   !disk_buffer_reusable_segment_check(currdisk, seg)) {
...
int disk_buffer_check_segments(currdisk, currioreq, buffer_reading)
disk *currdisk;
ioreq_event *currioreq;
int* buffer_reading;
{
...
void disk_buffer_set_segment(currdisk, currdiskreq)
disk *currdisk;
diskreq *currdiskreq;
{
   segment     *seg = currdiskreq->seg;
...
int disk_buffer_attempt_seg_ownership(currdisk, currdiskreq)
disk *currdisk;
diskreq *currdiskreq;
{
   segment     *seg = currdiskreq->seg;
...
int disk_buffer_get_max_readahead(currdisk, seg, curr)
disk *currdisk;
segment *seg;
ioreq_event *curr;
{
   ...
   if (currdisk->contread == BUFFER_NO_READ_AHEAD) {
      ...
      bandptr = disk_translate_lbn_to_pbn(currdisk, endreq, MAP_FULL, 
					  &cylno, &surface, &blkno);

      disk_get_lbn_boundaries_for_track(currdisk, bandptr, cylno, surface, 
					&startlbn, &endlbn);

      if (seg->startblkno < startlbn) {
          return(endlbn + bandptr->blkspertrack);
      } else {
	  return(endlbn);
      }
   }
   ...
   if (currdisk->keeprequestdata > 0) {
      if ((curr->bcount >= seg->size) || ((seg->size - curr->bcount) < currdisk->minreadahead)) {
